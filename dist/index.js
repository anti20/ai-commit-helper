...
import { access, lstat, mkdir, readdir, readFile, rm, unlink, writeFile, } from "node:fs/promises";
...
function parseUserConfig(contents) {
    ...
    if (rawConfig.styleCommits !== undefined) {
        if (typeof rawConfig.styleCommits !== "number" ||
...
async function selectAiProvider(requestedProvider, providers) {
    if (requestedProvider !== "auto") {
        const provider = providers.find((availableProvider) => availableProvider.name === requestedProvider);
        if (!provider) {
...
async function commitWithMessage(commitMessage) {
    ...
    const messagePath = join(tmpdir(), `ai-commit-helper-message-${randomUUID()}.txt`);
    try {
        await writeFile(messagePath, `${commitMessage.trim()}\n`, "utf8");
...
async function uninstallNpmGlobalPackages() {
    ...
    const packageNames = Array.from(new Set(["ai-commit-helper", packageName].filter(Boolean)));
    let removed = false;
...
function printUninstallSummary(summary) {
    ...
    console.log(`removed install directory: ${summary.removedInstallDirectory ? "yes" : "no"}`);
    console.log(`removed npm global package: ${summary.removedNpmGlobalPackage ? "yes" : "no"}`);
...
async function run(options) {
    ...
    try {
        ...
        console.log(chalk.cyan(`Available AI providers: ${availableProviders.length > 0 ? providerNames(availableProviders) : "none"}`));
        let aiProvider;
        try {
        ...
        if (!aiProvider) {
            console.error(chalk.red("No AI provider found. Install/login to Codex CLI or set OPENAI_API_KEY."));
            process.exitCode = 1;
            return;
        }
        ...
        try {
            ...
            console.error(chalk.red(`Error generating summary with ${aiProvider.name}: ${getErrorMessage(error)}`));
            process.exitCode = 1;
        }
    }
    catch (error) {
